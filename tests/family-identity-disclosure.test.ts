...
const source = readFileSync("src/components/overview/family-identity-cell.tsx", "utf8");

describe("family identity disclosure", () => {
  it("uses one accessible button for the icon and family identity text", () => {
  ...
  it("keeps the 44 px disclosure icon target while making family text clickable", () => {
    ...
    expect(source).toContain("{family.familyNameZhTw}");
    expect(source).toContain("{family.dexRangeZhTw}");
