  ...
  it("does not let unreleased or unknown variants become keep decisions", () => {
    for (const row of allRows()) {
      if (row.releaseStatus !== "RELEASED") {
        expect(row.initialDecision, row.id).toBe(row.releaseStatus === "UNKNOWN" ? "HOLD_FOR_NOW" : "TRANSFER_CANDIDATE");
        expect(row.initialDisposition, row.id).toBe(row.releaseStatus === "UNKNOWN" ? "TRUE_DATA_PENDING" : "NOT_APPLICABLE_OR_UNRELEASED");
