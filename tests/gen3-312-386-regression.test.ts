...
describe("Gen3 #312-#386 canonical and graph regression", () => {
  ...
  it("matches the independent canonical fixture for every species and Hoenn form", () => {
    ...
    const variantRecords = forms312386.flatMap((form) =>
    ...
    expect(validateGen3FormCompleteness(forms312386, variantRecords, { min: 312, max: 386 })).toEqual([]);
    ...
    expect(forms312386.some((form) => form.id === "351-hoenn" || form.id === "386-hoenn")).toBe(false);
  ...
  it("rejects a batch identity typo instead of accepting a self-generated expectation", () => {
    ...
    expect(validateGen3FormCompleteness(brokenForms, [], { min: 312, max: 386 })).toEqual(
      expect.arrayContaining([expect.stringContaining("313-hoenn Traditional Chinese form name mismatch")]),
    );
  });
  ...
  it("keeps Primal Kyogre/Groudon labels separate from Mega Rayquaza", () => {
    expect(specialVariants312386.find((variant) => variant.id === "382-hoenn-mega")?.nameZhTw).toBe("原始蓋歐卡");
    expect(specialVariants312386.find((variant) => variant.id === "383-hoenn-mega")?.nameZhTw).toBe("原始固拉多");
    expect(specialVariants312386.find((variant) => variant.id === "384-hoenn-mega")?.nameZhTw).toBe("Mega 烈空坐");
    const dashboard = JSON.parse(readFileSync("site-data/dashboard.json", "utf8")) as Array<Record<string, unknown>>;
    const visibleFields = [
