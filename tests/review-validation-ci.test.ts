...
describe("review validation CI contract", () => {
  ...
  it("runs review consistency in PR and production build workflows", () => {
    for (const path of [
      ".github/workflows/verify-pages-pr.yml",
      ".github/workflows/deploy-pages.yml",
    ]) {
      const workflow = readFileSync(path, "utf8");
      expect(workflow).toContain("- name: Review consistency");
      expect(workflow).toContain("run: npm run review:validate");
    }
  });
});
