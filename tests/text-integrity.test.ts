...
describe("visible text integrity and cross-generation target provenance", () => {
  ...
  it("keeps current manifest, runtime, and review visible text clean", () => {
    ...
    const historicalReviewArchives = /(?:family-aggregation-20260718|001-(?:151|181|211|241)-recalibration)\.json$/;
  const files = roots
    .flatMap((root) => jsonFiles(root))
    .filter((file) => !historicalReviewArchives.test(file));
    const issues = files.flatMap((file) => findTextIntegrityIssues(loadJson(file), relative(process.cwd(), file)));
    expect(issues, issues.slice(0, 12).map((issue) => `${issue.path}: ${issue.value}`).join("\n")).toEqual([]);
  });

  it("uses the corrected names, regions, and evolution edges", () => {
    const manifest = loadJson("research_notes/cross-generation-evolution-targets.json") as { targets: Array<Record<string, unknown>>; paths: Array<Record<string, unknown>> };
    const targetById = new Map(manifest.targets.map((target) => [`${String(target.dexNumber).padStart(3, "0")}-${String(target.formKey).toLowerCase()}`, target]));
    const names = {
      "982-paldea": "\u571f\u9f8d\u7bc0\u7bc0",
      "461-other": "\u746a\u72c3\u62c9",
      "473-other": "\u8c61\u7259\u8c6c",
    ...
    for (const [id, name] of Object.entries(names)) expect(targetById.get(id)?.nameZhTw, id).toBe(name);
    const expectedEdges = [
      ["206-johto", "982-paldea"], ["215-johto", "461-other"], ["221-johto", "473-other"],
      ["234-johto", "899-hisui"], ["217-johto", "901-hisui"],
    ];
    for (const [fromFormId, toFormId] of expectedEdges) {
      expect(manifest.paths.some((path) => path.fromFormId === fromFormId && path.toFormId === toFormId), `${fromFormId}->${toFormId}`).toBe(true);
    }
    expect(manifest.targets.filter((target) => Number(target.generation) >= 4 && target.formKey === "KANTO")).toEqual([]);
