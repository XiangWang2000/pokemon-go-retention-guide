...
function scopeNumber(value: number) { return String(value).padStart(3, "0"); }
const registryMinDex = BATCH_REGISTRY[0]?.minDex;
const registryMaxDex = BATCH_REGISTRY.at(-1)?.maxDex;
if (registryMinDex === undefined || registryMaxDex === undefined) throw new Error("The published batch registry must not be empty.");
const registryScope = `${scopeNumber(registryMinDex)}-${scopeNumber(registryMaxDex)}`;
if (registryScope !== CURRENT_DATA_SCOPE) throw new Error(`Current data scope ${CURRENT_DATA_SCOPE} does not match the published registry ${registryScope}.`);
const recalibrationBase = `review/${registryScope}-recalibration`;
const preRecomputeMaxDex = BATCH_REGISTRY.filter((entry) => entry.import.phase === "pre-recompute").at(-1)?.maxDex;
if (preRecomputeMaxDex === undefined) throw new Error("The published batch registry must contain a pre-recompute phase.");
const preRecomputeScope = `${scopeNumber(registryMinDex)}-${scopeNumber(preRecomputeMaxDex)}`;
const preRecomputeRecalibrationBase = `review/history/${preRecomputeScope}-recalibration`;
...
export function isExpectedReleaseGeneratedPath(filePath: string) {
  ...
  if (!normalized || normalized.startsWith("/") || segments.some((segment) => segment === "." || segment === "..")) return false;
  if (CURRENT_RELEASE_CONTRACT.snapshot.generatedRoots.some((root) => normalized.startsWith(`${root}/`))) return true;
  ...
  return normalized === CURRENT_RELEASE_CONTRACT.review.preRecomputeRecalibrationJsonPath || normalized === CURRENT_RELEASE_CONTRACT.review.preRecomputeRecalibrationMarkdownPath;
}
