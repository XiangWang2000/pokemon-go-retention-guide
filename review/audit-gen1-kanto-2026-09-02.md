...
- 本次修正的核心是 **PvP 快照新鮮度、ML 投資語意與 Mega 寶石海星推出狀態**。
...
舊版只要某 BattleVariant 被判定有 Master League 用途，家族摘要就統一顯示「ML 高 IV 投資候選」，容易讓 #101～250 的物種看起來像主流投資目標。

本 PR 改成：

- ML 物種榜 #1～100：`ML 主力候選（物種榜 #X；高 IV 投資）`
- ML 物種榜 #101～250：`ML 次要候選（物種榜 #X；少量高 IV 即可）`
- 其他人工限定用途：`ML 特定用途候選`

個體層仍沿用 ML 高 IV 篩選原則；這項修改只是在「先判斷物種值不值得投資」的層級避免過度保留。
