...
- `docs/history/legacy-sites-migration.md`：退休 Sites/Vinext/Cloudflare 遷移的歷史紀錄，非現行操作手冊。
...
- `scripts/pages/serve-pages.mjs`：本機提供 Pages `out/` artifact 的 HTTP 入口。

## 資料流程

1. 研究證據進入 `research_notes/sources/` 與 `data/sources/`，並保留來源版本與查閱資訊；早期人工筆記歸檔於 `research_notes/history/`。
2. Prisma schema、migration 與 seed 定義結構化資料與資料庫操作。
3. `scripts/data/validate-data.ts`、`scripts/review/generate-review.ts` 與 remediation scripts 驗證資料並產出審核資訊。
4. `scripts/release/prepare-release-snapshot.ts` 將 `scripts/release/generate-static-snapshot.ts` 的輸出寫入 staging，
   由 `scripts/release/verify-release.ts` 驗證後才 promote 到 `site-data/`、`public/data/`、`public/exports/` 與 Excel 交付資料。
...
| `src/app/`、`src/components/` | Next.js 介面、metadata 與 static routes。  |
| `prisma/`                     | Schema、migration 與 seed。                |
| `scripts/data/`               | 資料匯入、重算與完整性驗證。               |
| `scripts/review/`             | 審核報告產生與一致性驗證。                 |
| `scripts/release/`            | Snapshot 產生、promotion 與 release 驗證。 |
| `scripts/pages/`              | Pages build、serve 與 HTTP smoke。         |
| `tests/`                      | 單元與整合測試。                           |

## Source of Truth 與產生物

- 研究資料與來源快照必須能追溯到 `research_notes/sources/`、`data/sources/` 或受控 import。
- `review/` 根目錄只保存目前 release contract 的報告；舊 checkpoint 放在 `review/history/`。
- 現行維護文件放在 `docs/`；退休架構與歷史畫面放在 `docs/history/`。
