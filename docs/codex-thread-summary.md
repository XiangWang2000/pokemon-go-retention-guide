...
- Release preparation now uses `src/config/release-contract.ts` and `scripts/verify-release.ts` as the shared current-scope verification contract. `release:snapshot` generates into a repository-local staging directory, validates the complete output, and promotes the artifact set transactionally; PR, Pages deploy, and manual release workflows call the same verifier. The release workflow is manual/ref-driven rather than guarded by a publish-branch name or a version grep.
