...
- The existing Prisma-to-snapshot parity test remains the integration guard and now compares the logical read model whenever a readable local SQLite database is available, independently of the manifest's binary database hash. Persistence-only IV recommendation timestamps are projected out at the Dashboard boundary and object-key order is canonicalized, while `verifyStaticSnapshot()` and `verifyRelease()` still enforce the manifest-pinned database path, bytes, and SHA-256 whenever a database is supplied.
