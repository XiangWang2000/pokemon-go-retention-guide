...
> 歷史紀錄，非現行操作手冊。Sites／Vinext／Cloudflare Worker runtime、purge helper、`public/_headers` 與 `sites:*` npm commands 已移除；目前請依 [`../github-pages.md`](../github-pages.md) 使用 GitHub Pages。
...
```powershell
npm run data:recompute -- --max 251
...
The GitHub Pages deployment is documented separately in `../github-pages.md`. Its Pages-only
