...
async function seedExternalForms(url: string) {
  ...
  try {
    ...
    const ownedFormIds = new Set(definitions.flatMap((definition) => definition.forms.map((form) => form.id)));
...
async function verifyBatch(url: string, batch: string) {
  ...
  try {
    ...
    assert(species === definition.species.length, `${batch}: species count ${species} does not match source.`);
    assert(forms === definition.forms.length, `${batch}: form count ${forms} does not match source.`);
    assert(actualIds.size === expectedIds.size && [...expectedIds].every((id) => actualIds.has(id)), `${batch}: persisted variants differ from the generic plan.`);
    assert(released === expectedReleased, `${batch}: released variant count ${released} does not match source.`);
    assert(categories === expectedIds.size * 7, `${batch}: category count is not complete.`);
    assert(evaluations === expectedIds.size, `${batch}: retention evaluation count is not complete.`);
    assert(rows.length === definition.forms.length, `${batch}: persisted form identity count is not complete.`);
  } finally {
    await db.$disconnect();
  }
}
...
async function verifyCanonicalTransition(url: string) {
  ...
  try {
    ...
    assert(!legacy && edges.length === 1, "The Roserade identity or edge is not canonical and unique.");
  } finally {
    await db.$disconnect();
  }
}
...
console.log(`Generic Gen4 persistence importer verified for ${gen4Entries.map((entry) => entry.key).join(", ")}.`);
