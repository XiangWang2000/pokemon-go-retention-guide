...
import { validateReviewConsistency } from "../review/validate-review-consistency";
...
export async function verifyRelease({
  ...
  const manifest: SnapshotManifest = await verifyStaticSnapshot({
    snapshotRoot,
    databaseRoot,
    databaseUrl,
  });
...
if (scriptPath?.endsWith("/scripts/release/verify-release.ts")) {
  ...
  if ([snapshotRoot, reviewRoot, databaseRoot, databaseUrl].some((value) => value === "")) {
