...
async function runSnapshotGenerator(stagingRoot: string) {
  await new Promise<void>((resolve, reject) => {
    const child = spawn(npx, ["tsx", "scripts/release/generate-static-snapshot.ts"], {
      ...
      env: {
        ...process.env,
        DATABASE_URL: CURRENT_RELEASE_CONTRACT.snapshot.databaseUrl,
...
async function main() {
  ...
  try {
    ...
    await verifyRelease({
      ...
      reviewRoot: root,
      databaseRoot: root,
      databaseUrl: CURRENT_RELEASE_CONTRACT.snapshot.databaseUrl,
    });
...
if (scriptPath?.endsWith("/scripts/release/prepare-release-snapshot.ts")) {
  main().catch((error) => {
