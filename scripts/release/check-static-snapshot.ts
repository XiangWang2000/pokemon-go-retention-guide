...
export async function verifyStaticSnapshot({
  ...
  const databaseLocation = resolveDatabaseLocation(databaseUrl, databaseRoot);
  ...
  if (await exists(databaseLocation.absolutePath)) {
    ...
    assert(database.byteLength > 0, "Snapshot 來源資料庫是空檔。");
    assert(
      database.byteLength === manifest.sourceDatabase.bytes,
      "Snapshot 來源資料庫大小與 snapshot 不一致。",
    );
    assert(
      sha256(database) === manifest.sourceDatabase.sha256,
      "Snapshot 來源資料庫已變更，請先執行 npm run release:snapshot。",
    );
  }
...
if (scriptPath?.endsWith("/scripts/release/check-static-snapshot.ts")) {
  if (snapshotRoot === "" || databaseRoot === "" || databaseUrl === "") {
    console.error("--root, --database-root and --database-url require a value.");
    process.exitCode = 1;
  } else {
