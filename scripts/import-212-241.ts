...
const officialResearch = JSON.parse(
  readFileSync(new URL("../research_notes/official-212-241.json", import.meta.url), "utf8"),
) as OfficialResearch;
...
function variantReleased(formId: string, variantKey: VariantKey) {
  ...
  if (variantKey === "SHADOW" || variantKey === "PURIFIED") {
    return releasedShadowForms212241.has(formId);
  }
  if (variantKey === "MEGA") return releasedMegaForms212241.has(formId);
  if (variantKey === "DYNAMAX") return releasedDynamaxForms212241.has(formId);
  if (variantKey === "GIGANTAMAX") return releasedGigantamaxForms212241.has(formId);
  return false;
}
...
function findRanks(
  ...
  const speciesId = pvpokeSpeciesId212241(form, variantKey === "SHADOW");
...
function initialDecision(variantKey: VariantKey, released: boolean, ranks: RankResult[], formId: string) {
  ...
  if (variantKey === "DYNAMAX" || pveUseLevels212241[formId] === "CORE_INVESTMENT") {
    return "KEEP" as const;
  }
  if (pveUseLevels212241[formId]) return "CONDITIONAL_KEEP" as const;
...
function pvpSourceRows(variants: VariantRecord[], rankMap: Map<string, RankResult[]>) {
  return variants.flatMap((variant) =>
    (rankMap.get(variant.id) ?? []).map((rank) => ({
      ...
      speciesKey: pvpokeSpeciesId212241(variant.form, variant.variantKey === "SHADOW"),
...
async function rebuildBatch(rankings: Map<LeagueKey, RankingRow[]>) {
  ...
  await prisma.pokemonSpecies.createMany({
    data: species212241.map((species) => ({
  ...
  await prisma.pokemonForm.createMany({
    data: forms212241.map((form) => {
      const species = species212241.find((item) => item.dexNumber === form.dexNumber)!;
      return {
        id: form.id,
  ...
  if (migratedStubIds212241.size) {
    const oldStubIds = [...migratedStubIds212241];
    ...
    data: evolutionPairs212241
      ...
      availabilityNotesZhTw:
        toFormId === "199-galar"
          ...
          : "此進化路徑已在 #212～#241 整合資料中核對；批次外目標以正式 stub 保留。",
      requiresEvent: false,
  ...
  for (const special of specialVariants212241) {
    const form = forms212241.find((item) => item.id === special.formId)!;
    variants.push({ id: special.id, form, variantKey: special.variantKey, released: special.released });
  }
  if (species212241.length !== 30 || forms212241.length !== 30 || variants.length !== 124) {
    throw new Error(`#212～#241 靜態計數不符 30 species／31 forms（含 Galar stub）／121 variants。`);
  }
  ...
  const pveTier = (formId: string) =>
    pveUseLevels212241[formId] === "CORE_INVESTMENT"
      ? "A"
      : pveUseLevels212241[formId] === "USABLE_OR_BUDGET"
        ? "B"
        : pveUseLevels212241[formId] === "SPECIAL_USE"
          ? "SPECIAL"
          : null;
  const rawRows = [
    ...
    return categories.map((category) => {
      ...
      if (category === "PVP") {
        ...
        if (!variant.released || ["DYNAMAX", "GIGANTAMAX"].includes(variant.variantKey)) {
          status = variant.released ? "NOT_APPLICABLE" : "UNRELEASED";
        } else if (variant.variantKey === "MEGA" || pveUseLevels212241[variant.form.id]) {
          status = "PARTIALLY_VERIFIED";
          provenance = "SOURCE_VERIFIED";
          materialToDecision = true;
          pveUseLevel = variant.variantKey === "MEGA" ? "SPECIAL_USE" : pveUseLevels212241[variant.form.id]!;
        ...
        if (variant.variantKey === "MEGA") {
          ...
          summaryZhTw = variant.released
            ...
            : "此 Mega 版本尚未推出。";
        } else if (variant.variantKey === "NORMAL" && releasedMegaForms212241.has(variant.form.id)) {
          status = "PARTIALLY_VERIFIED";
          provenance = "SOURCE_VERIFIED";
          materialToDecision = true;
        ...
        const hasReleasedMax = releasedDynamaxForms212241.has(variant.form.id) || releasedGigantamaxForms212241.has(variant.form.id);
        if (isMaxVariant) {
          status = variant.released ? "VERIFIED" : "UNRELEASED";
          provenance = variant.released ? "SOURCE_VERIFIED" : "MANUAL_CURATED";
          materialToDecision = variant.released;
        ...
        const outgoing = evolutionPairs212241.some(([from]) => from === variant.form.id);
  ...
  for (const link of officialEvidenceLinks.filter((candidate) =>
    ...
    if (
      ...
      releasedMegaForms212241.has(linkedVariant.form.id) &&
    ...
    return {
      ...
      megaSummaryZhTw:
        variant.variantKey === "MEGA"
          ...
          : releasedMegaForms212241.has(variant.form.id) && variant.variantKey === "NORMAL"
      ...
      evolutionSummaryZhTw: evolutionPairs212241.some(([from]) => from === variant.form.id)
  ...
  await prisma.evaluationRuleTrace.createMany({
    data: variants.map((variant) => {
      ...
      return {
        ...
        explanationZhTw: "#212～#241 批次初步評估，待共用重算流程依跨世代 family graph 再確認。",
      };
    }),
  });
  ...
  await prisma.changeLog.createMany({
    data: [
      ...
      },
      {
        ...
        fieldName: "members",
        previousValue: "#041～#042＋#106?#107?#236?#237",
  ...
  if (counts[0] !== 30 || counts[1] !== 30 || counts[2] !== 124 || counts[3] !== 868) {
    throw new Error(`本批計數錯誤：${counts.join("/")}，預期 30/30/124/868。`);
  }
  console.log(`#212～#241 匯入完成：本批 ${counts.join("/")}；raw rows ${rawRows.length}。`);
}
