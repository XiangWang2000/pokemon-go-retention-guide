...
async function rebuildBatch(rankings: Map<LeagueKey, RankingRow[]>) {
  ...
  if (species212241.length !== 30 || forms212241.length !== 30 || variants.length !== 124) {
    throw new Error(
      `#212～#241 靜態計數不符 30 species／31 forms（含 Galar stub）／121 variants。`,
    );
  }
  ...
  for (const variant of variants) {
    rankMap.set(
      ...
      variant.released && (variant.variantKey === "NORMAL" || variant.variantKey === "SHADOW")
        ? findLegacyRanks(variant.form, variant.variantKey, rankings, pvpokeSpeciesId212241)
        : [],
    );
  }
    ...
    pveUseLevels212241[formId] === "CORE_INVESTMENT"
      ...
      : pveUseLevels212241[formId] === "USABLE_OR_BUDGET"
        ...
        : pveUseLevels212241[formId] === "SPECIAL_USE"
          ...
          : null;
  const rawRows = [
    ...buildLegacyPvpSourceRows(variants, rankMap, pvpokeSpeciesId212241, pvpokeCommit, checkedAt),
    ...
    return categories.map((category) => {
      ...
      if (category === "PVP") {
        ...
        if (!variant.released || ["DYNAMAX", "GIGANTAMAX"].includes(variant.variantKey)) {
          ...
          pveUseLevel =
            variant.variantKey === "MEGA" ? "SPECIAL_USE" : pveUseLevels212241[variant.form.id]!;
          summaryZhTw =
        ...
        if (variant.variantKey === "MEGA") {
          ...
          releasedMegaForms212241.has(variant.form.id)
        ) {
          status = "PARTIALLY_VERIFIED";
          provenance = "SOURCE_VERIFIED";
          materialToDecision = true;
        ...
        const hasReleasedMax =
          releasedDynamaxForms212241.has(variant.form.id) ||
          releasedGigantamaxForms212241.has(variant.form.id);
        if (isMaxVariant) {
          status = variant.released ? "VERIFIED" : "UNRELEASED";
          provenance = variant.released ? "SOURCE_VERIFIED" : "MANUAL_CURATED";
          materialToDecision = variant.released;
...
async function main() {
  await upsertLegacySources(prisma, officialResearch, checkedAt, "第 #212～#241 批次來源研究表。");
