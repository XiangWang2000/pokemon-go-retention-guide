...
function expectedCounts(definition: ReturnType<typeof getGen6BatchDefinition>) {
...
export async function generateGen6Review(batch: string) {
  const entry = getBatchByKey(batch);
  if (entry.import.adapter !== "gen6") throw new Error(`Batch ${batch} is not owned by Gen6.`);
  ...
  const checks = [
    { name: "Gen6 exact form identity", result: actualFormIds.size === expectedFormIds.size && [...expectedFormIds].every((id) => actualFormIds.has(id)) ? "PASS" : "FAIL" },
    { name: "Gen6 batch boundary counts", result: new Set(rows.map((row) => row.dexNumber)).size === entry.maxDex - entry.minDex + 1 && rows.length === expected.battleVariants ? "PASS" : "FAIL" },
    { name: "Gen6 exact release-state preservation", result: rows.filter((row) => row.releaseStatus === "RELEASED").length === expected.released && rows.filter((row) => row.releaseStatus === "UNKNOWN").length === expected.unknown ? "PASS" : "FAIL" },
    { name: "Gen6 no TRUE_DATA_PENDING", result: rows.every((row) => row.assessmentDisposition !== "TRUE_DATA_PENDING") ? "PASS" : "FAIL" },
    { name: "Gen6 owning-family coverage", result: [...expectedFormIds].every((formId) => families.some((family) => family.members.some((member) => member.form.formId === formId))) ? "PASS" : "FAIL" },
  ] as const;
  const failed = checks.filter((check) => check.result !== "PASS");
  if (failed.length) throw new Error(`Gen6 ${batch} review checks failed: ${failed.map((check) => check.name).join(", ")}`);
  const payload = {
  ...
  const lines = [
    `# Pokémon GO Retention Guide #${batch} Gen6 publication review`, "",
    ...
    `- status: ${payload.status}`, "", "## Gen6 exact-form publication checks", "",
...
async function main() {
  ...
  if (!batch || process.argv.length > 3) throw new Error("Usage: tsx scripts/review/generate-review-gen6.ts <registered Gen6 batch>.");
  const payload = await generateGen6Review(batch);
...
if (scriptPath?.endsWith("/scripts/review/generate-review-gen6.ts")) main().catch((error) => { console.error(error instanceof Error ? error.message : error); process.exitCode = 1; });
