...
export async function readExpectedPagesSmokeContract() {
  ...
  assert(
    Array.isArray(auditSummary.rows) &&
...
async function checkWorkbook(siteUrl, expectedWorkbook) {
  ...
  assert(
    workbook.length === expectedWorkbook.bytes,
    `${url} returned ${workbook.length} bytes instead of ${expectedWorkbook.bytes}.`,
  );
  ...
  assert(
    sha256 === expectedWorkbook.sha256,
    `${url} SHA256 ${sha256} does not match expected ${expectedWorkbook.sha256}.`,
  );
}
...
export async function smokePagesHttp(
  ...
  if (basePath) {
    assert(
      homeHtml.includes(basePath),
      `Home HTML does not contain the Pages base path ${basePath}.`,
    );
  }
  ...
  assert(
    typeof payload.dataVersion === "string" && payload.dataVersion.length > 0,
    "Served home.json has no dataVersion.",
  );
