...
export async function generateGen4Review(batch: string) {
  ...
  const checks = [
    ...
    },
    {
      ...
      result: species.size === entry.maxDex - entry.minDex + 1 && rows.length === expectedRows ? "PASS" : "FAIL",
    },
    {
      name: "Gen 4 released variant boundary",
      result: rows.filter((row) => row.releaseStatus === "RELEASED").length === expectedReleasedVariants(definition) ? "PASS" : "FAIL",
    },
    {
  ...
  if (failedChecks.length) {
    throw new Error(`Gen4 ${batch} review checks failed: ${failedChecks.map((check) => check.name).join(", ")}`);
  }

  const strategies = ["KEEP_TARGETS", "SELECTIVE_KEEP", "MOSTLY_TRANSFER", "HOLD_FOR_NOW"];
  const strategyCounts = Object.fromEntries(
    strategies.map((strategy) => [strategy, families.filter((family) => family.retentionStrategy === strategy).length]),
  );
  ...
  const scopedHolds = families
    ...
    .map((family) => ({ familyId: family.familyId, members: family.members.map((member) => member.form.formId) }));
...
async function main() {
  ...
  if (!batch || process.argv.length > 3) throw new Error("Usage: tsx scripts/generate-review-gen4.ts <registered Gen4 batch>.");
...
if (scriptPath?.endsWith("/scripts/generate-review-gen4.ts")) {
  main().catch((error) => {
