...
async function assertExistingDecisionsStable(
  ...
  if (changed.length || missing.length) {
    throw new Error(
      [
        "Existing #001-151 retention conclusions changed or are missing from the r19 snapshot.",
        ...changed.slice(0, 20),
...
async function main() {
      ...
      if (criticalIssue && !affectingExistingIssue) {
        ...
        await tx.dataIssue.upsert({
          ...
          create: {
            ...
            status: "OPEN",
            batchKey: "001-241-recalibration",
            messageZhTw,
  ...
  const highRiskDex = new Set([
    ...
    212, 213, 214, 215, 216, 217, 220, 221, 227, 229, 230, 233, 234, 236, 237, 238, 239, 240, 241,
  ]);
  const report = {
    scope: `${dexMin}-${dexMax}`,
  ...
  await writeFile(
    "review/001-241-recalibration.json",
  ...
  await writeFile("review/001-241-recalibration.md", `${reportLines.join("\r\n")}\r\n`, "utf8");
  console.log(
    JSON.stringify(
      {
        scope: `${dexMin}-${dexMax}`,
