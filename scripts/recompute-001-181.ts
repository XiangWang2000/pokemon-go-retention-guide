...
async function assertExistingDecisionsStable(
  ...
  if (changed.length || missing.length) {
    throw new Error(
      [
        "Existing #001-151 retention conclusions changed or are missing from the r17 snapshot.",
        ...changed.slice(0, 20),
...
async function main() {
      ...
      if (criticalIssue && !affectingExistingIssue) {
        ...
        await tx.dataIssue.upsert({
          ...
          create: {
            ...
            status: "OPEN",
            batchKey: "001-181-recalibration",
            messageZhTw,
  ...
  await writeFile(
    "review/001-181-recalibration.json",
  ...
  await writeFile("review/001-181-recalibration.md", `${reportLines.join("\r\n")}\r\n`, "utf8");
  console.log(
    JSON.stringify(
      {
        scope: `${dexMin}-${dexMax}`,
