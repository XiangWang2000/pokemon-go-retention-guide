...
async function main() {
  ...
  const outputDirectory = dexMax === CURRENT_DATA_MAX_DEX ? "review" : "review/history";
  await mkdir(outputDirectory, { recursive: true });
  await writeFile(
    outputDirectory + "/001-" + dexMax + "-recalibration.json",
  ...
  await writeFile(
    outputDirectory + "/001-" + dexMax + "-recalibration.md",
    `${reportLines.join("\r\n")}\r\n`,
    "utf8",
  );
  console.log(
