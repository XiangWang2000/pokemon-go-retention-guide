...
import { getGen6BatchDefinition } from "../../src/data/batch-gen6";
import { buildGen6ImportPlan, type Gen6ImportPlanRow, type Gen6PlanLeague, type Gen6PvpRankingRow, type Gen6RankingSnapshots } from "../../src/data/gen6-import-plan";
...
const leagueMeta: Record<Gen6PlanLeague, { cp: number; sourceId: string; label: string }> = {
...
async function upsertSource(prisma: PrismaClient, source: ResearchSource, fallbackCheckedAt: string, fallbackType = "SECONDARY") {
  ...
  const data = {
    ...
    notes: "Imported from dated Gen6 candidate evidence during formal publication.",
  };
...
async function upsertPvPokeSources(prisma: PrismaClient) {
  for (const league of Object.keys(leagueMeta) as Gen6PlanLeague[]) {
    ...
    const data = {
      ...
      sourceLanguage: "en",
      sourceSummaryZhTw: "固定 PvPoke 排名快照，供第六世代正式匯入使用；exact form 與共享 mapping 邊界沿用 candidate evidence。",
...
async function readRankings(): Promise<Gen6RankingSnapshots> {
  ...
  for (const league of Object.keys(leagueMeta) as Gen6PlanLeague[]) {
    ...
    result[league] = JSON.parse(json.replace(/^\uFEFF/, "")) as Gen6PvpRankingRow[];
  }
  return result;
}
function releaseBoolean(status: Gen6ImportPlanRow["releaseStatus"]) {
  return status === "RELEASED" ? true : status === "UNRELEASED" ? false : null;
}
function pvpSummary(row: Gen6ImportPlanRow) {
...
function reason(row: Gen6ImportPlanRow) {
...
async function upsertSpeciesAndForms(prisma: PrismaClient, definition: ReturnType<typeof getGen6BatchDefinition>) {
  for (const species of definition.species) {
    const id = `species-${String(species.dexNumber).padStart(3, "0")}`;
    const data = { dexNumber: species.dexNumber, nameEn: species.nameEn, nameZhTw: species.nameZhTw, generation: 6, familyKey: species.familyKey };
  ...
  for (const form of definition.forms) {
    ...
    const data = {
      ...
      evolvesFromFormId: null, evolutionFamilyNotesZhTw: "第六世代 exact-form identity；只 materialize candidate 已核對的正式進化邊。",
...
async function materializeEvolutionPaths(prisma: PrismaClient, definition: ReturnType<typeof getGen6BatchDefinition>) {
  const formIds = new Set((await prisma.pokemonForm.findMany({ select: { id: true } })).map((form) => form.id));
  assertEvolutionPathEndpoints(formIds, definition.evolutionPairs, `Gen6 ${definition.key}`);
  for (const [fromFormId, toFormId] of definition.evolutionPairs) {
    await upsertEvolutionPath(prisma, {
      id: `evolution-gen6-${definition.key}-${fromFormId}-${toFormId}`,
      fromFormId, toFormId,
      evolutionMethodZhTw: "Pokémon GO 正式進化路徑；型態切換、Fusion 與不可互換 form 不建立進化邊。",
      availabilityNotesZhTw: `第六世代 ${definition.key} identity evidence 已核對此正式進化 endpoint。`,
      requiresEvent: false, verifiedAt: checkedAt,
    });
  }
}
async function writeBattleVariants(prisma: PrismaClient, plan: readonly Gen6ImportPlanRow[]) {
...
async function writeEvidence(prisma: PrismaClient, definition: ReturnType<typeof getGen6BatchDefinition>, pveSourceByUrl: Map<string, string>, plan: readonly Gen6ImportPlanRow[], knownSources: Set<string>) {
  const raw: Array<Record<string, unknown>> = [];
  for (const row of plan) {
    for (const rank of row.ranks) raw.push({
      id: `raw-gen6-${definition.key}-${row.id}-${rank.league.toLowerCase()}`, battleVariantId: row.id, category: "PVP", status: rank.mappingMode === "EXACT" ? "VERIFIED" : "PARTIALLY_VERIFIED",
      ...
      rawNotes: pvpSummary(row), seasonOrVersion: `PvPoke commit ${pvpokeCommit}`, extractionMethod: "固定 rankings JSON index + 1；mapping mode 由 Gen6 candidate evidence 鎖定。",
      reproducible: true, sourceId: leagueMeta[rank.league].sourceId, checkedAt,
    });
    if (row.pveEvidence) raw.push({
      id: `raw-gen6-${definition.key}-${row.id}-pve`, battleVariantId: row.id, category: "PVE", status: "PARTIALLY_VERIFIED", league: "NOT_APPLICABLE",
    ...
    if (row.maxEvidence) raw.push({
      id: `raw-gen6-${definition.key}-${row.id}-max`, battleVariantId: row.id, category: "MAX_BATTLE", status: "PARTIALLY_VERIFIED", league: "NOT_APPLICABLE",
  ...
  const evaluations = plan.map((row) => ({
    id: `gen6-${definition.key}-eval-${row.id}`, battleVariantId: row.id, finalDecision: row.initialDecision, provenance: "MANUAL_CURATED" as const,
    ...
    assessmentDisposition: row.initialDisposition, reviewNotesZhTw: `由 Gen6 ${definition.key} formal publication importer 依 candidate evidence 產生。`,
  }));
  ...
  await prisma.evaluationRuleTrace.createMany({ data: plan.map((row) => ({
    id: `gen6-${definition.key}-trace-${row.id}`, evaluationId: `gen6-${definition.key}-eval-${row.id}`,
    ...
    matched: true, resultDecision: row.initialDecision, explanationZhTw: "第六世代正式匯入保留 exact form／variant 邊界；UNKNOWN 不會被改寫成 UNRELEASED。",
  })) });
  ...
  const add = (row: Gen6ImportPlanRow, sourceId: string, category: string, usageZhTw: string) => {
    if (!knownSources.has(sourceId)) return;
    const c = `category-${row.id}-${category}`; const e = `gen6-${definition.key}-eval-${row.id}`;
...
export async function runImportGen6(batch: string, databaseUrl = getDatabaseUrl()) {
  const entry = getBatchByKey(batch);
  if (entry.import.adapter !== "gen6") throw new Error(`Batch ${batch} is not owned by the Gen6 adapter.`);
  const definition = getGen6BatchDefinition(batch);
  ...
  try {
    ...
    const plan = buildGen6ImportPlan(definition, await readRankings());
    ...
    await prisma.changeLog.upsert({
      where: { id: `gen6-${batch}-batch` },
      create: { id: `gen6-${batch}-batch`, entityType: "Batch", entityId: batch, fieldName: "status", previousValue: null, newValue: "PUBLISHED", sourceId, changeReasonZhTw: `將第六世代 ${batch} candidate evidence 接入正式 BattleVariant importer。`, changedAt: checkedAt, rulesVersion: RULES_VERSION },
      update: { newValue: "PUBLISHED", sourceId, changeReasonZhTw: `重建第六世代 ${batch} 正式 BattleVariant 資料。`, changedAt: checkedAt, rulesVersion: RULES_VERSION },
    });
    return { prisma, plan };
  } catch (error) { await prisma.$disconnect(); throw error; }
}
export async function closeGen6Import(result: { prisma: PrismaClient }) { await result.prisma.$disconnect(); }

async function main() {
  assertDisposableDatabase(getDatabaseUrl());
  const batch = process.argv[2];
  if (!batch || process.argv.length > 3) throw new Error("Usage: tsx scripts/data/import-gen6.ts <registered Gen6 batch>.");
  const result = await runImportGen6(batch);
  ...
  finally { await closeGen6Import(result); }
}
const scriptPath = process.argv[1]?.replaceAll("\\", "/");
if (scriptPath?.endsWith("/scripts/data/import-gen6.ts")) main().catch((error) => { console.error(error instanceof Error ? error.message : error); process.exitCode = 1; });
