...
import type {
  ...
  Gen3SpecialVariant,
  PveUseLevel,
} from "../../src/data/batch-gen3-types";
import {
  forms282311,
...
import { assertDisposableDatabase } from "./legacy-import-shared";
import {
...
type Category = "PVP" | "PVE" | "ROCKET" | "GYM" | "MEGA" | "MAX_BATTLE" | "EVOLUTION_VALUE";
...
function readResearch(batch: BatchDefinition) {
  return JSON.parse(
    readFileSync("research_notes/sources/official-" + batch.batch + ".json", "utf8"),
  ) as OfficialResearch;
}
  ...
  for (const source of research.sources) {
    for (const variantId of source.supports) {
      const isDirectShadowRoster =
        shadowRosterSourceIds.has(source.id) && /-(shadow|purified)$/.test(variantId);
      addLink({
        sourceId: source.id,
        variantId,
    ...
    for (const support of source.supports) {
      ...
      for (const formId of descendants) {
        ...
        addLink({
          ...
          category: "ROCKET",
          usageZhTw:
            "Shadow 可正常進化機制來源（derived/inherited closure）；由已發布 Shadow 起始物種沿正式 evolution path 推導，不代表來源 roster 直接列出。",
        });
      }
    }
  }
    ...
    for (const [fromFormId, toFormId] of batch.evolutionPairs) {
      ...
      for (const variantSuffix of ["shadow", "purified"] as const) {
        addLink({
          ...
          usageZhTw:
            "該物種實際 evolution path 來源（formal evolution edge）；只標示此 Shadow/Purified 目標可沿該正式路徑取得。",
        });
      }
    }
  }
...
export async function runImport(batchName: string) {
      ...
      if (category === "PVP") {
        ...
        if (variant.variantKey === "MEGA") {
          ...
          summaryZhTw = variant.released
            ? variantNameZhTw + " 已推出；只保留實際要投入的候選，與普通、暗影及 Max 分開。"
            : variantNameZhTw + " 尚未推出。";
        } else if (
          variant.variantKey === "NORMAL" &&
          batch.releasedMegaForms.has(variant.form.id)
        ) {
          status = "PARTIALLY_VERIFIED";
          provenance = "SOURCE_VERIFIED";
          materialToDecision = true;
          summaryZhTw = isPrimalFormId(variant.form.id)
            ? "此普通型態是" + variantNameZhTw + "的基底；不把原始回歸用途回推成全家族必留。"
            : "此普通型態是已推出 Mega 的基底；不把 Mega 用途回推成全家族必留。";
        } else {
          status = "NOT_APPLICABLE";
          summaryZhTw = isPrimalFormId(variant.form.id)
            ? "此版本不是原始回歸型態；家族有原始回歸不代表所有版本都必須保留。"
            : "此版本不是 Mega 型態；家族有 Mega 不代表所有成員都必須保留。";
        }
      } else if (category === "MAX_BATTLE") {
        const maxVariant = variant.variantKey === "DYNAMAX" || variant.variantKey === "GIGANTAMAX";
        const hasReleasedMax =
          batch.releasedDynamaxForms.has(variant.form.id) ||
          batch.releasedGigantamaxForms.has(variant.form.id);
        if (maxVariant) {
          status = variant.released ? "VERIFIED" : "UNRELEASED";
          provenance = variant.released ? "SOURCE_VERIFIED" : "MANUAL_CURATED";
          materialToDecision = variant.released;
    ...
    return {
      ...
      requiredMovesSummaryZhTw: pvpUseful
        ? "依固定快照優先核對：" +
          [
            ...new Set(
              result.ranks.filter((rank) => rank.rank <= 250).flatMap((rank) => rank.moves),
            ),
          ].join("／")
  ...
  if (batch.batch === "252-281") {
    ...
    await prisma.changeLog.create({
      data: {
        ...
        sourceId: "EVOLUTION-HOENN-20260809",
        changeReasonZhTw:
          "土居忍士、鐵面忍者與脫殼忍者保留共同家族語意；脫殼忍者不被誤建成遊戲內不存在的直接進化路徑。",
        changedAt: checkedAt,
        rulesVersion: RULES_VERSION,
      },
    });
  ...
  const counts = {
    species: await prisma.pokemonSpecies.count({
      where: { dexNumber: { gte: batch.start, lte: batch.end } },
    }),
    forms: await prisma.pokemonForm.count({
      where: { species: { dexNumber: { gte: batch.start, lte: batch.end } } },
    }),
    variants: await prisma.battleVariant.count({
      where: { pokemonForm: { species: { dexNumber: { gte: batch.start, lte: batch.end } } } },
    }),
    categories: await prisma.categoryEvaluation.count({
      where: {
        battleVariant: {
          pokemonForm: { species: { dexNumber: { gte: batch.start, lte: batch.end } } },
        },
      },
    }),
  };
  if (
    counts.species !== batch.species.length ||
    counts.forms !== batch.forms.length ||
    counts.variants !== variants.length ||
    counts.categories !== variants.length * categories.length
  ) {
    throw new Error("第三世代批次計數錯誤：" + JSON.stringify(counts));
  }
  console.log(
    JSON.stringify(
      { batch: batch.batch, counts, rawRows: rawRows.length, sources: research.sources.length },
      null,
      2,
    ),
  );
...
async function main() {
  ...
  if (!batch || process.argv.length > 3) {
    throw new Error("Usage: tsx scripts/data/import-gen3.ts <batch>");
  }
  await runImport(batch);
}

const scriptPath = process.argv[1]?.replaceAll("\\", "/");
if (scriptPath?.endsWith("/scripts/data/import-gen3.ts")) {
  main()
    .catch((error) => {
