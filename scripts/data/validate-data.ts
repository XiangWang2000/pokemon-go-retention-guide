...
async function collectTypeScriptFiles(root: string): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) =>
        entry.isFile() && /\.(?:ts|tsx)$/.test(entry.name) && !entry.name.endsWith(".test.ts"),
    )
...
async function main() {
  ...
  for (const file of [
    "research_notes/sources/official-001-030.json",
    "research_notes/sources/battle-001-015.json",
    "research_notes/sources/battle-016-030.json",
    "research_notes/sources/official-031-060.json",
    "research_notes/sources/battle-031-060.json",
    "research_notes/sources/official-061-090.json",
    "research_notes/sources/battle-061-090.json",
    "research_notes/sources/official-091-120.json",
    "research_notes/sources/battle-091-120.json",
    "research_notes/sources/official-121-151.json",
    "research_notes/sources/battle-121-151.json",
    "research_notes/sources/official-152-181.json",
    "research_notes/sources/battle-152-181.json",
    "research_notes/sources/official-182-211.json",
    "research_notes/sources/battle-182-211.json",
    "research_notes/sources/official-212-241.json",
    "research_notes/sources/battle-212-241.json",
    "research_notes/sources/official-242-251.json",
    "research_notes/sources/battle-242-251.json",
    "research_notes/sources/official-252-281.json",
    "research_notes/sources/battle-252-281.json",
    ...
    errors.push(
      ...textIssues.map((issue) => `Corrupted source text at ${issue.path}: ${issue.value}`),
    );
  }
  for (const file of (await readdir("review")).filter((name) => name.endsWith(".json"))) {
    const parsed = JSON.parse(
      (await readFile(join("review", file), "utf8")).replace(/^\uFEFF/, ""),
    ) as {
      dataVersion?: string;
    };
    ...
    errors.push(
      ...textIssues.map((issue) => `Corrupted active review text at ${issue.path}: ${issue.value}`),
    );
  }
