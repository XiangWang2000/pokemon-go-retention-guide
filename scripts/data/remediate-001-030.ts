...
} from "../../src/data/remediation-policy";
...
async function applyReleaseStatuses(official: OfficialResearch) {
  ...
  for (const form of forms) {
    ...
    const formStatus: ReleaseStatus = resolveReleaseStatus(
      mapOfficialStatus(source?.releaseStatus),
    );
...
async function upsertCategory(data: {
  ...
  const requestedSourceIds = [...new Set(data.sourceIds ?? [])];
  const existingSources = new Set(
    (
      await prisma.sourceReference.findMany({
        where: { id: { in: requestedSourceIds } },
        select: { id: true },
      })
    ).map((source) => source.id),
  );
  const missingSourceIds = requestedSourceIds.filter((sourceId) => !existingSources.has(sourceId));
  if (data.material && missingSourceIds.length) {
    throw new Error(
      `${data.variantId}/${data.category} 缺少 material category 來源：${missingSourceIds.join(", ")}`,
    );
  }
  const provenance: EvaluationProvenance = resolveCategoryProvenance({
    status: data.status,
    explicit: data.provenance,
    linkedSourceCount: existingSources.size,
  });
    ...
    if (release === "UNRELEASED") {
      for (const category of categories) {
        await upsertCategory({
          ...
          status: "UNRELEASED",
          material: false,
          summaryZhTw: "已確認此戰鬥版本尚未推出，本類別目前不需完整評估。",
          sourceIds: releaseSources,
...
async function createReviewIssues(invalidPvpVariants: Set<string>) {
  ...
  const variants = await prisma.battleVariant.findMany({
    where: { pokemonForm: { species: { dexNumber: baseDexRange } } },
    include: { categoryEvaluations: { orderBy: { category: "asc" } } },
  });
  ...
  for (const variant of variants) {
    ...
    if (variant.releaseStatus === "UNKNOWN") {
      const { affectsFamily, messageZhTw } = unknownReleaseIssueDetails(variant.variantKey);
      issues.push({
        type: "UNKNOWN_RELEASE_STATUS",
        message: messageZhTw,
        affects: affectsFamily,
        action: "查找 Pokémon GO 官方公告、官方新聞或可核對日期的正式推出紀錄。",
        identity: stableReviewIssueKey({ type: "UNKNOWN_RELEASE_STATUS" }),
      });
    }
    ...
    if (invalidPvpVariants.has(variant.id)) {
      ...
      issues.push({
        ...
        identity: stableReviewIssueKey({ type: "UNREPRODUCIBLE_RANK", category: "PVP" }),
      });
    }
    ...
    if (optional.length) {
      issues.push({
        ...
        identity: stableReviewIssueKey({
          type: "OPTIONAL_DATA_MISSING",
          categories: optional.map((item) => item.category),
        }),
      });
    }
    if (holdForNow && !hasVariantScopedUnknownRelease && !issues.some((issue) => issue.affects)) {
      issues.push({
        ...
        identity: stableReviewIssueKey({ type: "RULE_NOT_COVERED" }),
      });
    }
    for (const issue of issues) {
      const id = `issue-${sanitize(variant.id)}-${RULES_VERSION}-${sanitize(issue.identity)}`;
...
async function writeMetrics(before: Map<string, string>) {
  ...
  const currentEvaluations = await prisma.retentionEvaluation.findMany({
    where: {
      rulesVersion: RULES_VERSION,
      battleVariant: { pokemonForm: { species: { dexNumber: { gte: 1, lte: 30 } } } },
    },
    select: { id: true, battleVariantId: true, finalDecision: true, reasonZhTw: true },
    orderBy: [{ generatedAt: "desc" }, { id: "desc" }],
  });
  const currentEvaluationByVariant = new Map<string, (typeof currentEvaluations)[number]>();
  for (const evaluation of currentEvaluations) {
    if (!currentEvaluationByVariant.has(evaluation.battleVariantId)) {
      currentEvaluationByVariant.set(evaluation.battleVariantId, evaluation);
    }
  }
  const holdForNowReasons = [...currentEvaluationByVariant.values()]
    .filter((evaluation) => evaluation.finalDecision === "HOLD_FOR_NOW")
    .map(({ battleVariantId, reasonZhTw }) => ({ battleVariantId, reasonZhTw }))
    .sort((left, right) => left.battleVariantId.localeCompare(right.battleVariantId));
