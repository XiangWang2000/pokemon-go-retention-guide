...
import { assertDisposableDatabase, getDatabaseUrl, resolveDatabaseLocation } from "../../src/lib/database";
...
const crossGenerationManifestPath =
...
await run(npx, ["tsx", "scripts/data/recompute-retention.ts", "--max", String(recomputeMaxDex)]);
for (const batch of postRecomputeBatches) {
  await runBatchImport(batch);
}
await run(npm, ["run", "data:materialize-iv"]);
